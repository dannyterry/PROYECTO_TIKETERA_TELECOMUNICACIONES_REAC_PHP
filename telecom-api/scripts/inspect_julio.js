...
async function inspectJulio() {
  try {
    console.log("=== INSPECCIÓN A FONDO JULIO 2026 ===\n");

    const oficial = {
      averias: { asig: 981, fin: 741, canc: 240, ef: 75.54 },
      postventa: { asig: 173, fin: 121, canc: 52, ef: 69.94 },
      total: { asig: 1154, fin: 862, canc: 292 }
    };

    // 1. Conteo total de órdenes en Julio 2026 por estado
    const [totales] = await pool.query(`
      ...
      ORDER BY total DESC
    `);
    console.log("1. Total de órdenes en Julio por Estado:", totales);
    ...
    // 3. Revisar Conjuntas PEXT en Julio
    ...
    `);
    console.log(`\n3. Conjuntas PEXT en Julio (${conjuntas.length}):`);
    ...
    // 4. Órdenes con ADICIONAL / WIFI PRO / SPLITTER / MESH en Julio
    ...
    `);
    console.log(`\n4. Órdenes Especiales en Julio (${especiales.length}):`);
