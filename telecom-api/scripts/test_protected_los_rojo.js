...
async function testProtectedLosRojoRule() {
  try {
    console.log("=== PROBANDO REGLA CON PROTECCIÓN DE LOS ROJO EN TODOS LOS MESES ===\n");
    ...
    const oficial = {
      ...
      9: { av_asig: 520, av_fin: 416, av_ef: 80.00, pv_asig: 89, pv_fin: 69, pv_ef: 77.53 }
    };
    ...
    for (let m = 1; m <= 9; m++) {
      ...
      const [rows] = await pool.query(`
        SELECT 
          -- Postventa con Protección de LOS ROJO
          COUNT(CASE WHEN (
      ...
      const exactPv = (r.pv_asig === of.pv_asig && r.pv_fin === of.pv_fin);

      console.log(`📌 ${meses[m-1].toUpperCase()}:`);
      console.log(`   POSTVENTA: WIN [${of.pv_asig} Asig, ${of.pv_fin} Fin, ${of.pv_ef}%] | BD [${r.pv_asig} Asig, ${r.pv_fin} Fin, ${efPv}%] -> ${exactPv ? '🏆 100.00% EXACTO' : '⚠️ DIF'}`);
