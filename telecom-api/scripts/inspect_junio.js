...
async function inspectJunio() {
  try {
    console.log("=== INSPECCIÓN A FONDO JUNIO 2026 ===\n");

    const oficial = {
      averias: { asig: 964, fin: 736, canc: 228, ef: 76.35 },
      postventa: { asig: 183, fin: 143, canc: 40, ef: 78.14 },
      total: { asig: 1147, fin: 879, canc: 268 }
    };

    // 1. Conteo total de órdenes en Junio 2026 por estado
    const [totales] = await pool.query(`
      SELECT 
        estado,
        COUNT(*) as total
      FROM ordenes
      WHERE YEAR(fecha_visita) = 2026 AND MONTH(fecha_visita) = 6
      ...
      ORDER BY total DESC
    `);
    console.log("1. Total de órdenes en Junio por Estado:", totales);

    // 2. Conteo preliminar aplicando las reglas
    ...
    // 3. Revisar Conjuntas PEXT en Junio
    ...
    `);
    console.log(`\n3. Conjuntas PEXT en Junio (${conjuntas.length}):`);
    ...
    // 4. Órdenes con ADICIONAL / WIFI PRO / SPLITTER / MESH en Junio
    ...
    `);
    console.log(`\n4. Órdenes Especiales en Junio (${especiales.length}):`);
    ...
    // 5. Tickets con visitas múltiples en Junio
    const [reiteradas] = await pool.query(`
      ...
      LIMIT 10
    `);
    console.log(`\n5. Tickets con visitas múltiples en Junio (${reiteradas.length} casos mostrados):`);
