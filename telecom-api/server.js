...
app.get('/api/almacen/tecnico-stock/:idTrabajador', async (req, res) => {
  try {
    ...
    if (!permitido) {
      return res.json({ permitido: false, materiales: [], seriesAsignadas: [] });
    }
    ...
    res.json({ materiales, seriesAsignadas });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
app.get(['/api/auditoria/metricas-gestores', '/auditoria/metricas-gestores'], async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(`
      ...
      GROUP BY a.id_usuario, a.usuario_nombre, a.rol_nombre, a.area
