...
const phpPath = "C:/xampp/htdocs/corporacionescepe/app/views/configuration/config/index.php";
        ...
        <?php foreach ($grupos as $grupo => $items): ?>
            ...
            <div class="<?= $colClass ?>">
                <div class="card shadow-sm border-0">
                    <div class="card-header bg-<?= $meta['color'] ?> text-white py-2 d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center gap-2">
                            <i class="mdi <?= $meta['icono'] ?> fs-5"></i>
                            <span class="fw-semibold"><?= $meta['titulo'] ?></span>
                        </div>
                        <?php if ($grupo === 'tiempo_real'): ?>
                            <button type="button" onclick="conectarGoogleLooker()" id="btn_conectar_google" class="btn btn-light btn-sm fw-bold text-danger d-inline-flex align-items-center gap-1 shadow-sm">
                                <i class="mdi mdi-google fs-6"></i> Conectar Cuenta Google (Looker Studio)
                            </button>
                        <?php endif; ?>
                    </div>
                    <div class="card-body">
...
<script>
function conectarGoogleLooker() {
    const btn = document.getElementById('btn_conectar_google');
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span> Abriendo Chrome...';
    }

    if (typeof Swal !== 'undefined') {
        Swal.fire({
            icon: 'info',
            title: 'Abriendo Google Chrome...',
            html: 'Se abrirá una ventana de Google Chrome en tu pantalla.<br><br><b>1.</b> Ingresa tu correo (<b>liquidacionescorpces@gmail.com</b>) y contraseña.<br><b>2.</b> Deja que cargue la página del reporte.<br><b>3.</b> La ventana se cerrará sola y la sesión quedará guardada.',
    ...
    fetch('http://localhost:3000/api/looker/launch-login', { method: 'POST' })
        ...
        .then(data => {
            if (btn) {
                btn.disabled = false;
                btn.innerHTML = '<i class="mdi mdi-google fs-6"></i> Conectar Cuenta Google (Looker Studio)';
            }
        })
        .catch(err => {
            console.error(err);
...
fs.writeFileSync(phpPath, cleanContent, 'utf8');
console.log('✅ index.php actualizado con handler directo!');
