$(document).ready(function () {
    ...
    $('<style>').prop('type', 'text/css').html(`
        ...
        /* Ocultar el selector de filas propio de DataTables
           (lo reemplazamos con nuestro select personalizado) */
        #tabla-ordenes_length { display: none !important; }
 
        /* Quitar margen extra del wrapper DT para que no haya
           espacio en blanco encima de la tabla */
        #tabla-ordenes_wrapper .row:first-child { margin-bottom: 4px !important; }
        #tabla-ordenes_wrapper .dt-buttons { display: none !important; }
    ...
    const tablaAjax = inicializarTabla(tabla, url_listar, columnas, {
        ...
        autoWidth: false,
        responsive: false,
        pageLength: 25,           // filas por defecto
        lengthChange: true,         // habilitar cambio de longitud
        dom: 'ftip',       // f=buscar, t=tabla, i=info, p=paginacion (sin B=botones, sin l=length nativo)
    });

    // Selector de filas personalizado en el toolbar
    $('#sel_page_length').on('change', function () {
        tablaAjax.page.len(parseInt(this.value)).draw();
    });
    ...
    $(document).on('click', '.cliente-info', function (e) {
        ...
        <div style="min-width:250px;">
            <b>Dirección:</b><br>
            ${el.data('direccion')}<br><br>

            <b>Georef:</b><br>
            ${el.data('georef')}<br><br>

            <b>Teléfono:</b><br>
            ${el.data('telefono')}<br><br>
    ...
    function cargarProductos(response) {
        ...
        // ─── Agrupar ─────────────────────────────
        const equipos = response.productos.filter(p =>
            p.categoria_liquidar === 'EQUIPO'
        );

        const materiales = response.productos.filter(p =>
            p.categoria_liquidar === 'MATERIAL'
        );
        ...
        <tr id="row_prod_${prod.id_producto}">
                        ...
                        <td class="text-center fw-semibold disp_prod_${prod.id_producto}">
                            ${stockActual}
                        </td>
    ...
    function renderTablaUsados() {
        ...
        productosUsados.forEach(function (p, idx) {
            ...
            if (parseInt(p.maneja_serie) === 1) {
                ...
                // ─── SOLO PARA DROP ───────────────── 
                if (p.nombre.toUpperCase() === 'DROP') {
                    cantidadCell += ` 
                    <div class="d-flex justify-content-center gap-1 mt-1"> 
                        <input type="number" 
                        class="form-control 
                        form-control-sm drop-inicio"
                        data-idx="${idx}" 
                        placeholder="Inicio" 
                        style="width:75px;">
    ...
    // Calcular cantidad automática para DROP 
    $(document).on('input', '.drop-inicio, .drop-fin', function () {
        const idx = $(this).data('idx');
        const fila = $(this).closest('tr');
        const inicio = parseInt(fila.find('.drop-inicio').val()) || 0;
        const fin = parseInt(fila.find('.drop-fin').val()) || 0;
        let total = 0;
        if (fin > inicio) { total = fin - inicio; }
        // Actualizar array 
        productosUsados[idx].cantidad = total;
        // Actualizar input visual 
        fila.find('.cantidad_usada').val(total);
    });
    ...
    function buscarSerie(serieEscaneada) {
        ...
        $('#tablaProductos tr').each(function () {
            ...
            const serie = fila.find('.serie_producto').text().trim();

            if (serie === serieEscaneada) {

                encontrado = true;

                // CLICK AUTOMÁTICO EN "+"
                fila.find('.btn-agregar-producto').click();
